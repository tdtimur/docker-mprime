...
RUN apt-get -qq update \
    && apt-get -qq install -y --no-install-recommends sudo wget \
...
USER mprime
WORKDIR /home/mprime
RUN wget -q http://www.mersenne.org/ftp_root/gimps/p95v2810.linux64.tar.gz -O ./mprime.tar.gz \
    && tar xzf mprime.tar.gz --absolute-names
RUN rm -f mprime.tar.gz
ENTRYPOINT ["./mprime"]
